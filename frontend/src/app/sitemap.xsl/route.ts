    ...
    <html lang="es">
      <head>
        ...
        <style>
          *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .hdr-title { font-size: 18px; font-weight: 700; letter-spacing: -0.02em; color: #fff4df; }
          .hdr-sub { font-size: 11px; color: rgba(206,223,182,0.7); letter-spacing: 0.07em; text-transform: uppercase; margin-top: 3px; }
          .hdr-count { text-align: right; }
          .hdr-num { font-size: 32px; font-weight: 800; color: #cedfb6; letter-spacing: -0.04em; line-height: 1; }
          .hdr-num-lbl { font-size: 10px; color: rgba(206,223,182,0.55); text-transform: uppercase; letter-spacing: 0.1em; margin-top: 4px; }
          /* INFO */
          .info {
            background: #fff;
          ...
          .pill {
            ...
            background: #f5f0e2; border-radius: 20px;
            padding: 5px 13px; font-size: 12px; font-weight: 500;
            color: #3a3a30; white-space: nowrap;
          }
          ...
          thead tr { background: #075627; }
          th {
            padding: 11px 14px; text-align: left;
            font-size: 10px; font-weight: 700;
            letter-spacing: 0.08em; text-transform: uppercase;
            color: #cedfb6; white-space: nowrap;
          }
          ...
          .url-link {
            ...
            color: #075627; font-weight: 500; font-size: 12px; text-decoration: none;
          }
          ...
          .pri-val { font-size: 12px; font-weight: 600; color: #333; width: 26px; text-align: right; flex-shrink: 0; }
          .pri-track { flex: 1; height: 5px; background: #e6e0d0; border-radius: 3px; overflow: hidden; }
          .pri-fill { height: 100%; border-radius: 3px; background: linear-gradient(90deg, #4a9c5e, #075627); }
          /* Freq */
          .freq { display: inline-block; border-radius: 4px; padding: 2px 8px; font-size: 11px; font-weight: 500; white-space: nowrap; }
          ...
          .alt-badge {
            ...
            background: #cedfb6; color: #03441d;
            font-size: 10px; font-weight: 700; border-radius: 10px; padding: 2px 8px; white-space: nowrap;
          }
          .img-badge {
            display: inline-flex; align-items: center; gap: 3px;
            background: #fef0ec; color: #b84422;
            font-size: 10px; font-weight: 700; border-radius: 10px; padding: 2px 8px; white-space: nowrap;
          }
          /* Footer */
          footer { text-align: center; padding: 20px 40px; font-size: 11px; color: #b0aa98; }
      ...
      <body>
        ...
        <header class="hdr">
          ...
          <div class="hdr-count">
            <div class="hdr-num"><xsl:value-of select="count(sm:urlset/sm:url)"/></div>
        ...
        <div class="info">
          ...
          La visualizaci&#xF3;n es solo para humanos y no interfiere en el indexado.
          &#xA0;&#xB7;&#xA0;
          <a href="https://www.sitemaps.org/protocol.html" target="_blank" rel="noopener">Protocolo Sitemap 0.9 &#x2197;</a>
        </div>

        <div class="stats">
          <span class="pill">
            <span class="dot dot-page"/>
            <xsl:value-of select="count(sm:urlset/sm:url[not(contains(sm:loc,'/productos/')) and not(contains(sm:loc,'/noticias/'))])"/>
            &#xA0;p&#xE1;ginas principales
          </span>
          <span class="pill">
          ...
          <span class="pill">
            ...
            &#xA0;<xsl:value-of select="count(sm:urlset/sm:url[count(image:image) &gt; 0])"/>&#xA0;con im&#xE1;genes
          </span>
        </div>
          ...
          <table>
            ...
            <tbody>
              <xsl:for-each select="sm:urlset/sm:url">
                <xsl:variable name="no-proto" select="substring-after(sm:loc, '//')"/>
                <xsl:variable name="path-raw" select="substring-after($no-proto, '/')"/>
                <xsl:variable name="display-path" select="concat('/', $path-raw)"/>
                ...
                <xsl:variable name="alt-count" select="count(xhtml:link[@rel='alternate' and @hreflang!='x-default'])"/>
                ...
                <tr>
                  ...
                  <td class="hide-sm">
                    <xsl:choose>
                      <xsl:when test="$locale = 'en'"><span class="loc-badge loc-en">EN</span></xsl:when>
                      <xsl:when test="$locale = 'fr'"><span class="loc-badge loc-fr">FR</span></xsl:when>
                      <xsl:when test="$locale = 'de'"><span class="loc-badge loc-de">DE</span></xsl:when>
                      <xsl:otherwise><span class="loc-badge loc-es">ES</span></xsl:otherwise>
                    </xsl:choose>
                  </td>
                  <td class="hide-sm">
                    <xsl:choose>
                  ...
                  <td class="hide-sm">
                    <xsl:choose>
                      ...
                      <xsl:otherwise>
                        <span class="freq"><xsl:value-of select="sm:changefreq"/></span>
                      </xsl:otherwise>
                    </xsl:choose>
                  </td>
                  <td class="date hide-sm"><xsl:value-of select="sm:lastmod"/></td>
                  <td style="text-align:center">
                    <xsl:if test="$alt-count &gt; 0">
                      <span class="alt-badge"><xsl:value-of select="$alt-count"/></span>
                    </xsl:if>
                  </td>
